...
import "test/risk-managers/unit-tests/StandardManager/TestStandardManagerBase.t.sol";

import "lyra-utils/encoding/OptionEncoding.sol";
import "forge-std/console2.sol";


contract SRMLiquidationSimBase is TestStandardManagerBase, JsonMechIO {
...
//    uint FinalPercentageReceived;
  ...
  function setupTestScenario(LiquidationSim memory data) internal {
    ...
    if (data.InitialPortfolio.BasePosition != 0) {
      balances[i++] = ISubAccounts.AssetBalance({
        asset: IAsset(address(wethAsset)),
        subId: 0,
  ...
  function updateFeeds(Feeds memory feedData) internal {
    ...
    ethFeed.setSpot(feedData.SpotPrice, feedData.SpotConfidence);
    ...
    for (uint i = 0; i < feedData.FeedExpiries.length; i++) {
      ...
      ethFeed.setForwardPrice(feedData.FeedExpiries[i], feedData.Forwards[i], feedData.ForwardConfidences[i]);
      ethFeed.setInterestRate(feedData.FeedExpiries[i], int64(feedData.Rates[i]), uint64(feedData.RateConfidences[i]));
    }
