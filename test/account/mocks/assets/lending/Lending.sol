...
contract Lending is IAsset, Owned {
  ...
  function _getBalanceFresh(uint accountId, int preBalance) internal view returns (int freshBalance) {
    ...
    return preBalance.multiplyDecimal(currentMarketIndex.toInt256()).divideDecimal(currentAccountIndex.toInt256());
  }
   ...
   */
  event FeeClaimed(address owner, uint reduceAmount, uint newAccruedFees);
