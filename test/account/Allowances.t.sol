...
contract Allowances is Test, LyraHelper {
  ...
  function testCannotTransferWithoutAllowance() public {    
    ...
    vm.startPrank(alice);
    vm.expectRevert(
      abi.encodeWithSelector(IAdvancedAllowance.NotEnoughSubIdOrAssetAllowances.selector, 
  ...
  function testCannotTransferWithPartialAllowance() public {    
    ...
    IAdvancedAllowance.SubIdAllowance[] memory subIdAllowances = new IAdvancedAllowance.SubIdAllowance[](2);
    subIdAllowances[0] = IAdvancedAllowance.SubIdAllowance({
      asset: IAsset(optionAdapter),
      subId: 0,
      positive: 4e17,
      negative: 0
    });
    subIdAllowances[1] = IAdvancedAllowance.SubIdAllowance({
      asset: IAsset(usdcAdapter),
      subId: 0,
      positive: 0,
      negative: 50e18
  ...
  function testNotEnoughAllowance() public {    
    ...
    vm.startPrank(bob);
    IAdvancedAllowance.AssetAllowance[] memory assetAllowances = new IAdvancedAllowance.AssetAllowance[](1);
    assetAllowances[0] = IAdvancedAllowance.AssetAllowance({
      asset: IAsset(usdcAdapter),
