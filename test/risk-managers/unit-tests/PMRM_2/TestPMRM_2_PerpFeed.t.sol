...
contract TestPMRM_2_PerpFeed is PMRM_2SimTest {
  function test_perpFeedAffectsPMRM_2() public {
    ...
    IPMRM_2.Portfolio memory alicePort = pmrm_2.arrangePortfolio(aliceAcc);

    _logPortfolio(alicePort, 0);

    assertEq(alicePort.perpValue, 0);
    assertEq(alicePort.perpPosition, -1e18, "alice should be short 1 perp");
    int mmPre = pmrm_2.getMargin(aliceAcc, false);

    IPMRM_2.Portfolio memory bobPort = pmrm_2.arrangePortfolio(bobAcc);
    ...
    alicePort = pmrm_2.arrangePortfolio(aliceAcc);
    assertEq(alicePort.perpValue, 100e18);

    _logPortfolio(alicePort, 0);

    int mmPost = pmrm_2.getMargin(aliceAcc, false);
