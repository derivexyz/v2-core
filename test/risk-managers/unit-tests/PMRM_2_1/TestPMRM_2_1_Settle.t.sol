...
contract TestPMRM_2_1_Settlement is PMRM_2_1TestBase {
  ...
  function setUp() public override {
    super.setUp();
    pmrm_2_1.setWhitelistedCallee(address(perpHelper), true);
    pmrm_2_1.setWhitelistedCallee(address(optionHelper), true);
  }
  ...
  function testCanSettlePerps() public {
    ...
    pmrm_2_1.settlePerpsWithIndex(aliceAcc);
  ...
  function testCanSettleWithManagerData() public {
    ...
    bytes memory data = abi.encode(address(pmrm_2_1), aliceAcc);
  ...
  function testCanSettleOptions() public {
    ...
    pmrm_2_1.settleOptions(option, aliceAcc);
    int cashAfter = _getCashBalance(aliceAcc);
  ...
  function testCanSettleOptionWithManagerData() public {
    ...
    bytes memory data = abi.encode(address(pmrm_2_1), address(option), aliceAcc);
    IBaseManager.ManagerData[] memory allData = new IBaseManager.ManagerData[](1);
  ...
  function testCannotCallUnWhitelistedContractInProcessManagerData() public {
    pmrm_2_1.setWhitelistedCallee(address(optionHelper), false);
  ...
  function testCanSettleOptionWithBadAddress() public {
    ...
    pmrm_2_1.settleOptions(IOptionAsset(address(mockPerp)), aliceAcc);
  }
