...
contract PMRM_2_1SimTest is PMRM_2_1TestBase {
    ...
    returns (ISubAccounts.AssetBalance[] memory balances)
  {
    ...
    string memory json = JsonMechIO.jsonFromRelPath("/test/risk-managers/unit-tests/PMRM_2_1/testScenarios.json");
    ...
    returns (ISubAccounts.AssetBalance[] memory balances)
  {
    uint referenceTime = block.timestamp;
    string memory json =
      JsonMechIO.jsonFromRelPath("/test/risk-managers/unit-tests/PMRM_2_1/testAndVerifyScenarios.json");
    ...
    pmrm_2_1.setBalances(aliceAcc, balances);
  ...
  function verify(Result memory results) internal {
    ...
    //      "asset contingencies"
    //    );
    assertApproxEqAbs(pmrm_2_1.getMargin(aliceAcc, true), results.initialMarginHand, 1e8, "Initial Margin Hand");
    assertApproxEqAbs(
      pmrm_2_1.getMargin(aliceAcc, false), results.maintenanceMarginHand, 1e8, "Maintenance Margin Hand"
    );
  }
}
