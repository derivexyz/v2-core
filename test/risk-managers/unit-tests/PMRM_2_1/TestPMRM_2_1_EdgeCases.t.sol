...
contract UNIT_TestPMRM_2_1_EdgeCases is PMRM_2_1SimTest {
  function testPMRM_2_1_perpTransfer() public {
  ...
  function testPMRM_2_1_unsupportedAsset() public {
    MockOption newAsset = new MockOption(subAccounts);
    // newAsset.setWhitelistManager(address(PMRM_2_1), true);
    ...
    vm.expectRevert(IPMRM_2_1.PMRM_2_1_UnsupportedAsset.selector);
    _doBalanceTransfer(aliceAcc, bobAcc, balances);
  }
  //  // TODO: we have no caps because of dampening, maybe we want one?
  //  function testPMRM_2_1_invalidSpotShock() public {
  //    IPMRM_2_1.Scenario[] memory scenarios = new IPMRM_2_1.Scenario[](1);
  //    scenarios[0] =
  //      IPMRM_2_1.Scenario({spotShock: 3e18 + 1, volShock: IPMRM_2_1.VolShockDirection.None, dampeningFactor: 1e18});
  //
  //    vm.expectRevert(IPMRM_2_1.PMRM_2_1_InvalidSpotShock.selector);
  ...
  function testPMRM_2_1_notFoundError() public {
    IPMRM_2_1.ExpiryHoldings[] memory expiryData = new IPMRM_2_1.ExpiryHoldings[](0);
    vm.expectRevert(IPMRM_2_1.PMRM_2_1_FindInArrayError.selector);
    pmrm_2_1.findInArrayPub(expiryData, 0, 0);
  }

  function testPMRM_2_1_noScenarios() public {
    // Cannot set no scenarios
    vm.expectRevert(IPMRM_2_1.PMRM_2_1_InvalidScenarios.selector);
    pmrm_2_1.setScenarios(new IPMRM_2_1.Scenario[](0));
  }

  function testPMRM_2_1_invalidGetMarginState() public {
    IPMRM_2_1.Scenario[] memory scenarios = new IPMRM_2_1.Scenario[](0);

    IPMRM_2_1.Portfolio memory portfolio;
    vm.expectRevert(IPMRMLib_2_1.PMRM_2_1L_InvalidGetMarginState.selector);
    pmrm_2_1.getMarginAndMarkToMarketPub(portfolio, true, scenarios);

    vm.expectRevert(IPMRMLib_2_1.PMRM_2_1L_InvalidGetMarginState.selector);
    pmrm_2_1.getMarginAndMarkToMarketPub(portfolio, true, scenarios);
  }

  function testPMRM_2_1_CannotTradeIfExceed_MaxAssets() public {
    uint expiry = block.timestamp + 1000;
    pmrm_2_1.setMaxAccountSize(10);
    ISubAccounts.AssetTransfer[] memory transfers = new ISubAccounts.AssetTransfer[](pmrm_2_1.maxAccountSize() + 1);
    ...
    vm.expectRevert(IPMRM_2_1.PMRM_2_1_TooManyAssets.selector);
    subAccounts.submitTransfers(transfers, "");
  }

  function testPMRM_2_1_CanTradeIfMaxAccountSizeDecreased() public {
    uint expiry = block.timestamp + 1000;
    pmrm_2_1.setMaxAccountSize(10);
  ...
  function testPMRM_2_1CannotInteractIfAuctionLive() public {
    ...
    pmrm_2_1.setLiquidation(mockAuction);
  ...
  function testRevertsIfUsingBadScenarioId() public {
    vm.expectRevert();
    pmrm_2_1.getMarginAndMarkToMarket(aliceAcc, true, 10000);
  }

  function testPMRM_2_1CanTradeMoreThanMaxAccountSizeIfAlreadyHadIt() public {
    ...
    pmrm_2_1.setMaxAccountSize(12);
    subAccounts.submitTransfers(transfers, "");
    assertEq(subAccounts.getAccountBalances(bobAcc).length, 12);
  }

  function testPMRM_2_1CanSettlePerpsIfBeyondMaxAssetSize() public {
    uint64 expiry = uint64(block.timestamp + 1 weeks);
    ...
    pmrm_2_1.settlePerpsWithIndex(bobAcc);

    assertEq(subAccounts.getAccountBalances(bobAcc).length, 13);
  }

  function testPMRM_2_1CanGetScenarioMTM() public {
    uint64 expiry = uint64(block.timestamp + 1 weeks);
