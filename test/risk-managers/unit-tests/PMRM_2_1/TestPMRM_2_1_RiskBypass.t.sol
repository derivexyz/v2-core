...
contract UNIT_TestPMRM_2_1_RiskBypass is PMRM_2_1SimTest {
  function testPMRM_2_1InsufficientMarginRegularTransfer() public {
    ISubAccounts.AssetBalance[] memory balances = setupTestScenarioAndGetAssetBalances(".BigOne");

    vm.expectRevert(IPMRM_2_1.PMRM_2_1_InsufficientMargin.selector);
    _doBalanceTransfer(aliceAcc, bobAcc, balances);
  }

  function testPMRM_2_1TransferPassesRiskChecks() public {
    ISubAccounts.AssetBalance[] memory balances = setupTestScenarioAndGetAssetBalances(".BigOne");
  ...
  function testPMRM_2_1_TrustedRiskAssessorBypass() public {
    ISubAccounts.AssetBalance[] memory balances = setupTestScenarioAndGetAssetBalances(".BigOne");

    // only bob's MTM is negative; alice's MTM is positive but IM is negative - yet trade goes through
    _depositCash(bobAcc, 200_000 ether);

    pmrm_2_1.setTrustedRiskAssessor(alice, true);
    vm.startPrank(bob);
  ...
  function testPMRM_2_1_TrustedRiskAssessorCanStillRevert() public {
    ISubAccounts.AssetBalance[] memory balances = setupTestScenarioAndGetAssetBalances(".BigOne");
  ...
  function testPMRM_2_1_riskReducingTrade() public {
    ISubAccounts.AssetBalance[] memory balances = setupTestScenarioAndGetAssetBalances(".BigOne");
    ...
    // Bob IM: ~168k
    int imPre = pmrm_2_1.getMargin(bobAcc, true);
    ...
    int imPost = pmrm_2_1.getMargin(bobAcc, true);
    assertLt(imPre, imPost);
  }

  function testPMRM_2_1_riskReducingTradeBasisContingency() public {
    ...
    pmrm_2_1.setTrustedRiskAssessor(address(this), false);
