...
contract Account is Allowances, ERC721, AccountStructs {
    ...
    mapping(uint256 => IManager) public manager;

    /// @dev accountId => asset => subId => BalanceAndOrder struct
    mapping(uint256 => mapping(IAsset => mapping(uint256 => BalanceAndOrder)))
        public balanceAndOrder;

    /// @dev accountId to non-zero assets array
    mapping(uint256 => HeldAsset[]) public heldAssets;
    ...
    modifier onlyOwnerOrManagerOrERC721Approved(
        address sender,
        uint256 accountId
    ) {
     ...
     */
    function createAccount(address owner, IManager _manager)
        external
     ...
     * @return newId ID of new account
     */
    function _createAccount(address owner, IManager _manager)
        internal
        returns (uint256 newId)
    {
        newId = ++nextId;
        ...
        onlyOwnerOrManagerOrERC721Approved(msg.sender, accountId)
    {
        ...
        (address[] memory uniqueAssets, uint256 uniqueLength) =
            _getUniqueAssets(heldAssets[accountId]);

        for (uint256 i; i < uniqueLength; ++i) {
        ...
        emit AccountManagerChanged(
            accountId, address(oldManager), address(newManager)
            );
    }
    ...
    function submitTransfer(
        ...
        external
    {
        (int256 fromDelta, int256 toDelta) = _transferAsset(assetTransfer);
        _managerHook(
            assetTransfer.fromAcc,
            msg.sender,
            AssetDeltaLib.getDeltasFromSingleAdjustment(
                assetTransfer.asset, assetTransfer.subId, fromDelta
            ),
            managerData
        );
        _managerHook(
            ...
            msg.sender,
            AssetDeltaLib.getDeltasFromSingleAdjustment(
                assetTransfer.asset, assetTransfer.subId, toDelta
            ),
            managerData
        );
    }
    ...
    function submitTransfers(
        ...
        external
    {
        uint256 transfersLen = assetTransfers.length;

        /* Keep track of seen accounts to assess risk once per account */
        uint256[] memory seenAccounts = new uint[](transfersLen * 2);

        // seen index => delta[]
        AssetDeltaArrayCache[] memory assetDeltas =
            new AssetDeltaArrayCache[](transfersLen * 2);

        uint256 nextSeenId = 0;

        for (uint256 i; i < transfersLen; ++i) {
            // if from or to account is not seens before, add to seenAccounts in memory
            (uint256 fromIndex, uint256 toIndex) = (0, 0);
            (nextSeenId, fromIndex) = ArrayLib.addUniqueToArray(
                seenAccounts, assetTransfers[i].fromAcc, nextSeenId
            );
            (nextSeenId, toIndex) = ArrayLib.addUniqueToArray(
                seenAccounts, assetTransfers[i].toAcc, nextSeenId
            );

            (int256 fromDelta, int256 toDelta) =
        ...
        for (uint256 i; i < nextSeenId; i++) {
            AccountStructs.AssetDelta[] memory nonEmptyDeltas =
                AssetDeltaLib.getDeltasFromArrayCache(assetDeltas[i]);
            _managerHook(
                seenAccounts[i], msg.sender, nonEmptyDeltas, managerData
            );
        }
    }
     ...
     * @param assetTransfer (fromAcc, toAcc, asset, subId, amount)
     */
    function _transferAsset(AssetTransfer memory assetTransfer)
        internal
        returns (int256 fromDelta, int256 toDelta)
    {
        if (assetTransfer.fromAcc == assetTransfer.toAcc) {
            revert AC_CannotTransferAssetToOneself(
                msg.sender, assetTransfer.toAcc
            );
        }
        ...
        (, int256 fromDelta_, bool fromAdjustmentNeedAllowance) =
            _adjustBalance(fromAccAdjustment, true);
        (, int256 toDelta_, bool toAdjustmentNeedAllowance) =
            _adjustBalance(toAccAdjustment, true);

        // if it's not ERC721 approved: spend allowances
        if (
            fromAdjustmentNeedAllowance
                && !_isApprovedOrOwner(msg.sender, fromAccAdjustment.acc)
        ) {
            _spendAllowance(
                fromAccAdjustment, ownerOf(fromAccAdjustment.acc), msg.sender
            );
        }
        if (
            toAdjustmentNeedAllowance
                && !_isApprovedOrOwner(msg.sender, toAccAdjustment.acc)
        ) {
            _spendAllowance(
                toAccAdjustment, ownerOf(toAccAdjustment.acc), msg.sender
            );
        }
    ...
    function managerAdjustment(AssetAdjustment memory adjustment)
        ...
        returns (int256 postAdjustmentBalance)
    {
        // balance is adjusted based on asset hook
        ...
        returns (int256 postAdjustmentBalance)
    {
        ...
        int256 delta;
        (postAdjustmentBalance, delta,) =
            _adjustBalance(adjustment, triggerAssetHook);
        console.log("After adjust balance");
        _managerHook(
            adjustment.acc,
            msg.sender,
            AssetDeltaLib.getDeltasFromSingleAdjustment(
                adjustment.asset, adjustment.subId, delta
            ),
            managerData
        );
        console.log("After manager hook");
    }
    ...
    function _adjustBalance(AssetAdjustment memory adjustment, bool triggerHook)
        internal
        returns (int256 postBalance, int256 delta, bool needAllowance)
    {
        BalanceAndOrder storage userBalanceAndOrder =
            balanceAndOrder[adjustment.acc][adjustment.asset][adjustment.subId];
        int256 preBalance = int256(userBalanceAndOrder.balance);

        // allow asset to modify final balance in special cases
        if (triggerHook) {
            (postBalance, needAllowance) =
                _assetHook(adjustment, preBalance, msg.sender);
        ...
        internal
    {
        manager[accountId].handleAdjustment(
            accountId, caller, deltas, managerData
        );
    }
    ...
    function _assetHook(
        ...
        internal
        returns (int256 finalBalance, bool needAllowance)
    {
     ...
     */
    function _addHeldAsset(uint256 accountId, IAsset asset, uint256 subId)
        internal
        returns (uint16 newOrder)
    {
        heldAssets[accountId].push(
            HeldAsset({asset: asset, subId: subId.toUint96()})
        );
     ...
     */
    function _removeHeldAsset(uint256 accountId, uint16 order) internal {
        /* swap order value if middle asset removed */
        uint256 heldAssetLen = heldAssets[accountId].length;

        // if the entry is not the last one: move the last asset to index #order
        if (order != heldAssetLen - 1) {
            HeldAsset memory assetToMove =
                heldAssets[accountId][heldAssetLen - 1];
            heldAssets[accountId][order] = assetToMove;

            // update the "order" field of the moved asset for an account
            balanceAndOrder[accountId][assetToMove.asset][uint256(assetToMove.subId)]
                .order = order;
        }
    ...
    function _getUniqueAssets(HeldAsset[] memory assets)
        internal
        pure
        returns (address[] memory uniqueAssets, uint256 length)
    {
        uniqueAssets = new address[](assets.length);

        for (uint256 i; i < assets.length; ++i) {
            length = ArrayLib.addUniqueToArray(
                uniqueAssets, address(assets[i].asset), length
            );
        }
    }
     ...
     */
    function getBalance(uint256 accountId, IAsset asset, uint256 subId)
        external
        view
     ...
     * @param accountId ID of account
     */
    function getAccountBalances(uint256 accountId)
        external
        view
        returns (AssetBalance[] memory assetBalances)
    {
        uint256 allAssetBalancesLen = heldAssets[accountId].length;
        assetBalances = new AssetBalance[](allAssetBalancesLen);
        for (uint256 i; i < allAssetBalancesLen; i++) {
            HeldAsset memory heldAsset = heldAssets[accountId][i];
            BalanceAndOrder memory userBalanceAndOrder = balanceAndOrder[accountId][heldAsset
                .asset][uint256(heldAsset.subId)];

            assetBalances[i] = AssetBalance({
                asset: heldAsset.asset,
                subId: uint256(heldAsset.subId),
                balance: int256(
                    userBalanceAndOrder.balance
                    )
            });
        }
     ...
     *
     */
    function _isApprovedOrOwner(address spender, uint256 accountId)
        internal
        view
        override
        returns (bool)
    ...
    error AC_CannotTransferAssetToOneself(address caller, uint256 accountId);

    error AC_CannotChangeToSameManager(address caller, uint256 accountId);
}
