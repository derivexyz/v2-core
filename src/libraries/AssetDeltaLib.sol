...
library AssetDeltaLib {
    ...
    function addToAssetDeltaArray(
        ...
        pure
    {
        for (uint256 i; i < cache.deltas.length;) {
            if (
                cache.deltas[i].asset
                    == delta.asset
                    && cache.deltas[i].subId
                    == delta.subId
            ) {
    ...
    function getDeltasFromArrayCache(
        AccountStructs.AssetDeltaArrayCache memory cache
    )
        internal
        pure
        returns (AccountStructs.AssetDelta[] memory)
    {
        AccountStructs.AssetDelta[] memory deltas =
            new AccountStructs.AssetDelta[](cache.used);
        for (uint256 i; i < deltas.length;) {
    ...
    function getDeltasFromSingleAdjustment(
        IAsset asset,
        uint256 subId,
        int256 delta
    )
    ...
    {
        AccountStructs.AssetDelta[] memory deltas =
            new AccountStructs.AssetDelta[](1);
